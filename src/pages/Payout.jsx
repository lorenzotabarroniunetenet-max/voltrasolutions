...
const statusBadge = s => {
  const map = {
    PAID: ['badge-success', 'Completato'],
    REJECTED: ['badge-fail', 'Rifiutato'],
    APPROVED: ['badge-info', 'Approvato'],
    PENDING: ['badge-pending', 'In verifica dal Comando']
  }
...
export default function Payout() {
  ...
  const [payoutEnabled, setPayoutEnabled] = useState(null) // null = loading
  ...
  useEffect(() => {
    ...
    // Controlla se il rimborso è abilitato
    fetch(`${BASE}/api/prop/payout-status`, { headers: h })
      .then(r => r.json())
      .then(d => setPayoutEnabled(d?.enabled ?? false))
      .catch(() => setPayoutEnabled(false))
  }, [])
  ...
  const confirm = async () => {
    ...
    try {
      ...
      setMsg('Richiesta inviata. Il Comando la verificherà entro 24-48 ore.')
  ...
  if (payoutEnabled === false) {
    return (
      <div>
        ...
        <div className="card" style={{ padding: 32, textAlign: 'center', border: '1px solid rgba(255,71,87,.2)', background: 'rgba(255,71,87,.04)' }}>
          <div style={{ fontSize: 36, marginBottom: 16 }}>🔒</div>
          <h2 style={{ fontSize: 20, fontWeight: 800, marginBottom: 8 }}>Rimborso non disponibile</h2>
          <p style={{ color: 'var(--muted)', fontSize: 14, lineHeight: 1.7, maxWidth: 400, margin: '0 auto 20px' }}>
            Il rimborso della missione non è ancora disponibile. Il Comando ti notificherà quando sarà possibile procedere.
          </p>
          <a href="/contact" style={{ display: 'inline-block', background: 'rgba(255,255,255,.05)', border: '1px solid var(--border)', color: 'var(--muted)', padding: '10px 20px', borderRadius: 8, textDecoration: 'none', fontSize: 13, fontWeight: 600 }}>
            Contatta il Comando
          </a>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 24, marginBottom: 20, background: 'rgba(180,255,57,0.03)', border: '1px solid rgba(180,255,57,0.15)' }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--muted)', lineHeight: 1.6 }}>
            Il rimborso viene processato manualmente dal Comando secondo i termini del contratto. Compila il modulo e verrai contattato entro 24-48 ore.
          </div>
        </div>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 20 }}
          ...
          <div className="card" style={{ maxWidth: 480, width: '100%', padding: 28 }} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 14, marginBottom: 16, lineHeight: 1.6 }}>
              Verifica che <strong>indirizzo</strong> e <strong>network</strong> siano corretti. Gli invii a indirizzi errati sono irrecuperabili.
            </p>
